...
- Two intentional local overrides of `al_folio_core` files: `_layouts/about.liquid` (lets the home page hide its title, and renders the social icons under the photo instead of at the bottom) and `_includes/header.liquid` (shows the name in the navbar on the home page too, where upstream hides it, drops upstream's bold first name, and turns the navbar into a fixed left sidebar on screens at least 992px wide, with the name on one line and the social icons under the menu links; below 992px the bar is always collapsed (name, theme toggle and menu button on one row; the menu panel holds the links and icons, and the closed menu is hidden with height/visibility because the theme's display rule is a layered !important), and the home page sets `social: false` so it shows no icons under the photo; it also hides the theme's copyright footer on every page, and keeps the theme toggle outside the collapsible menu so it sits next to the menu button on phones). A third override, `_includes/cv/render.liquid`, copies `al_folio_cv`'s `templates/cv/render.liquid` (1.0.2) with one change: the "Invited Talks and Seminars" section uses the awards layout (date ribbons; entries are `title`/`awarder`/`date` in `_data/cv.yml`). Avoid adding others; when bumping `al_folio_core` or `al_folio_cv`, diff these against the gems' versions.
